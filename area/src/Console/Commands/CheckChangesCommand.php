 ...
 * （纯确定性，不联网、不做语义判断）。分两层：schema 层 + 逻辑层。
 * 逻辑层是图级检查（不变量见 docs/changes-json-semantics.md §2.2，v3 收缩为五条）：
 *  I1 唯一键与退化边；I2 边端点真实；I3 跨层显式声明；I4 例外挂点与 id 空间；
 *  I5 diff 事实按侧认领；I7 id 复用显式化（含复用链禁环）。
 *  v3 简化（docs/area-precise-rollback-and-changes-v3.md §6）：side 由 state 派生、
 *  change_type 纯派生禁手写、attributes 只声明字段名（值从 csv 取）、evidence 为
 *  文件级证据池引用（下级边可继承单位级边）——I6 手写一致性核对整体消失。
...
class CheckChangesCommand extends Command
{
     ...
     * 跨字段条件（from≠to、跨层必填 cross_level_reason、单位级边证据自证等）由逻辑层强制。
     *
    ...
    protected function checkLogic(array $changes, ImportService $import): void
    {
        ...
        if ($diffPath !== null && is_file($diffPath)) {
            ...
            if (is_array($diff)) {
                $this->checkCoverageBySide($graph, $diff);
    ...
    protected function checkUniqueness(ChangesGraph $graph): void
    {
        ...
        foreach ($graph->nodes() as $node) {
            ...
            if (isset($seenNodes[$key])) {
                $this->errors[] = "changes[{$node['_index']}]（node）与 changes[{$seenNodes[$key]}] 重复：同一 (side={$node['side']}, id={$node['id']}) 只允许一条 node";
            }
     ...
     * 认领（旧侧事实不得由新侧字段认领）。
     *  - removed：旧侧认领（old node / edge from / old node exceptions）；
     *  - added：新侧认领（new node / edge to / new node exceptions）；
     *  - renamed / parent_changed（id 两版均在）：两侧都要认领。常规由一条
     *    continued node 认领（单侧书写，校验器派生对侧）；id_reuse 特判：
     *    由 retired + appeared(id_reuse=true) 节点对认领。
     *
     * @param  array<string, mixed>  $diff
     */
    protected function checkCoverageBySide(ChangesGraph $graph, array $diff): void
    {
        $oldClaimed = [];
        $newClaimed = [];
        ...
        foreach (($diff['provinces'] ?? []) as $province => $groups) {
            foreach (['added', 'removed', 'renamed', 'parent_changed'] as $kind) {
                foreach ($groups[$kind] ?? [] as $row) {
                    ...
                    $facts[$id] = [
                        "{$province}/{$kind} #{$id}",
                        in_array($kind, ['removed', 'renamed', 'parent_changed'], true),
                        in_array($kind, ['added', 'renamed', 'parent_changed'], true),
                    ];
                }
            }
        }

        foreach ($facts as $id => [$label, $needOld, $needNew]) {
            if ($needOld && ! isset($oldClaimed[$id])) {
                $this->errors[] = "覆盖率：diff 事实 {$label} 未被旧侧认领（retired/continued node / edge 的 from_id / retired node 的 exceptions）";
            }
            if ($needNew && ! isset($newClaimed[$id])) {
                $this->errors[] = "覆盖率：diff 事实 {$label} 未被新侧认领（appeared node / edge 的 to_id / appeared node 的 exceptions / continued node 派生对侧）";
            }
        }
    }
    ...
    /**
     * 版本一致：changes.json 的 version 与 diff.json 的 to_version 一致；
     * from_version 双侧均提供时也必须一致。
     *
     * @param  array<string, mixed>  $changes
     * @param  array<string, mixed>  $diff
     */
    protected function checkVersion(array $changes, array $diff): void
    {
        ...
        if (! empty($diff['from_version']) && ! empty($changes['from_version'])
            && $changes['from_version'] !== $diff['from_version']) {
            $this->errors[] = "版本不一致：changes.json 的 from_version（{$changes['from_version']}）≠ diff.json 的 from_version（{$diff['from_version']}）";
        }
    }
